...
ComplexNumber<NUM_T> ComplexParser<NUM_T>::parse (const std::string& str, std::function<int(int)> isNumber) {
    NUM_T re = 0.0;
    NUM_T im = 0.0;
    ...
    return ComplexNumber<NUM_T>{re, im};
}

template<class NUM_T>
bool ComplexParser<NUM_T>::readSign(const std::string& str, char& sign) {
    bool success = false;
...
template<class NUM_T>
NUM_T ComplexParser<NUM_T>::readReal(const std::string& realStr, const char sign) {
...
template<class NUM_T>
NUM_T ComplexParser<NUM_T>::readImag(const std::string& imagStr, const char sign) {
    if (imagStr.length() == 1) {
